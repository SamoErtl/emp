...
public class PogledZdravil extends AppCompatActivity  {
    ...
    String baseUrl = "http://zdravilnica.azurewebsites.net/IZdravila.svc/Zdravila";  // This is the API base URL ,  or Zdravila/{name}
    String url;  // This will hold the full URL which will include the username entered in the etGitHubUser.

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_pogled_zdravil);
    ...
    private void addToRepoList(String repoName, String lastUpdated) {
        ...
        String strRow = repoName + " Proizvajalec: " + lastUpdated;
                        ...
                        if (response.length() > 0) {
                            ...
                            for (int i = 0; i < response.length(); i++) {
                                try {
                                    ...
                                    String lastUpdated = jsonObj.get("Id_manu").toString();
                                    //dodaj hidden revile on click
                                    String description = jsonObj.get("Descr").toString();
                                    String inst = jsonObj.get("Inst").toString();
                                    String repoNameLat = jsonObj.get("NameLat").toString();
    ...
    public void getReposClickedEmpty(View v) {
        // Clear the repo list (so we have a fresh screen to add to)
        clearRepoList();
        // Call our getRepoList() function that is defined above and pass in the
        // text which has been entered into the etGitHubUser text input field.
